...
class Model:
    ...
    def calculate(self, a, b):
        ...
        r_r = int(FixedPoint(r_r,m=int(self.output_width/2),signed=False,overflow_alert='ignore',overflow='wrap'))
        r_i = int(FixedPoint(r_i,m=int(self.output_width/2),signed=False,overflow_alert='ignore'))
        r_bytes = r_r.to_bytes(byteorder='big',length=int(self.output_width/8/2),signed=False)
        i_bytes = r_i.to_bytes(byteorder='big',length=int(self.output_width/8/2),signed=False)
